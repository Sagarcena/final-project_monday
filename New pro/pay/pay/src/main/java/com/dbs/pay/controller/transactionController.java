...
public class transactionController 
{
	...
	public transactionController(transactionRespository transactionrepository, 
			customerRespository customerrepository,bankRespository bankrepository, 
			currencyRespository currencyrepository,messageRespository messagerepository, 
			transfertypesRespository transfertypesrepository) 
    {
		super();
    ...
    @PostMapping("/bc_url")
    public int bank_check(TransactionRequested request)
    {
		...
		{
			 return ToProcesstransaction(request);
    ...
    public int ToProcesstransaction(TransactionRequested request)
	{ 
		customer sender = customerrepository.findById((request.getSender_customerid())).get();
		...
		Double transferFee =0.0025 * request.getAmount() * curr.getConversionrate();
